...
CANDIDATE_PATH = ORCHESTRATION_DIR / "phase1c-task-alignment-v2-1-r1-candidate-manifest.json"
APPROVAL_ID = "1bb3a4e642058de0c81b3d0ae5274e21f8e54cbe468cf90338e5291f40a13d63"
EXPECTED_CANDIDATE_FILE = "e03120dc04f6032c13640bb3bbc13472198f0c94398198d41758234600202094"
...
def verify_candidate() -> tuple[dict[str, Any], dict[str, Any], set[str], dict[str, str]]:
    ...
    if any(path.exists() for path in (APPROVAL_PATH, ATTEMPT_PATH, RESULT_PATH, CAPTURE_DIR)):
    ...
    if candidate.get("candidate_type") != "TASK_ALIGNMENT_SAME_FIXTURE_CALIBRATION":
    ...
    if manifest.get("proposed_hard_cost_cap_usd") != "0.74":
        raise RuntimeError("COST_CAP_MISMATCH")
    if Decimal(manifest.get("hard_worst_case_cost_usd", "Infinity")) > Decimal("0.74"):
        raise RuntimeError("BUDGET_BLOCKED")

    fixture = import_fixture(
        Path(r"X:\01_codex_task.txt"), Path(r"X:\02_codex_report.txt"),
        Path(r"X:\03_manual_sol_review.txt"), project="bTest", historical_date="2026-08-13",
    )
    ...
    if actual_schema_hash != sha256_json(reviewer_output_schema("2.1")):
        raise RuntimeError("ACTUAL_SCHEMA_SOURCE_MISMATCH")
    expected_binding = dict(candidate["request_binding"])
    recalculated = {
    ...
    if recalculated != expected_binding:
...
def main() -> int:
    ...
    atomic_json(APPROVAL_PATH, {
        ...
        "candidate_file_sha256": EXPECTED_CANDIDATE_FILE, "approved_external_transmission": True,
        "approved_local_response_capture": True, "approved_cost_cap_usd": "0.74",
    ...
    safe = {name: value for name, value in result.items() if name not in {"error_metadata", "response_capture"}}
